...
public class JVidCap extends Frame implements ItemListener, ActionListener {
	...
	private void startMonitoring() {
		...
		if (checkAudio.getState()) {
			...
			int samplingRate = Integer
					.parseInt(comboSampling.getSelectedItem());
		...
		if (checkVideo.getState()) {
			...
			vf = new VideoFormat(encoding, size, Format.NOT_SPECIFIED, null,
					15f);
		}
		...
		if (datasource != null) {
			...
			MonitorControl mc = (MonitorControl) datasource
					.getControl("jmfsample.MonitorStream");
			if (mc != null) {
	...
	private void startCapture() {
		...
		try {
			MediaLocator ml = new MediaLocator("file:capture." + (outputType.equals("video.x_msvideo") ? "avi" : "mov"));
	...
	public void itemStateChanged(ItemEvent sce) {
		...
		if (source == comboSize || source == comboEncoding
				|| source == comboSampling || source == radio16bit
				|| source == radio8bit || source == radioMono
