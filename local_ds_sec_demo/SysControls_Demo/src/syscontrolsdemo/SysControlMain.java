...
public class SysControlMain 
{
		...
		try {
			...
			while((line = in.readLine()) != null)
			{
				if(line.startsWith("AUDIT="))
				{
					...
					ConnSingleton.getInstance().setAuditCursor(aCursor);
				}
				if(line.startsWith("STREAM="))
				{
					int sCursor = Integer.parseInt(line.replace("STREAM=", ""));
					ConnSingleton.getInstance().setStreamCursor(sCursor);
				}
			}
		}
		catch(Exception e)
	...
	private void setSettingsFile()
	{
		try {
			...
			out.write("AUDIT=" + ConnSingleton.getInstance().getAuditCursor());
			out.newLine();
			out.write("STREAM=" + ConnSingleton.getInstance().getStreamCursor());
		}
		catch (Exception e)
		{
